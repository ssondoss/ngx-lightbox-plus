...
export class LightboxComponent implements OnInit, AfterViewInit, OnDestroy, OnInit {
  ...
  private _registerImageLoadingEvent(): void {
    if (!this.options.isVideo) {
      ...
      //   };
        ...
        const trustedUrl: SafeResourceUrl = this._sanitizer.bypassSecurityTrustResourceUrl(src);
        iframe.src = trustedUrl as string;
        ...
        video.src = this._sanitizer.sanitize(SecurityContext.URL, src);
      }
  ...
  private _onLoadImageSuccess(): void {
    ...
    if (this.options.isVideo) {
      imageWidth = naturalImageWidth = this._imageElem.nativeElement.width;
      imageHeight = naturalImageHeight = this._imageElem.nativeElement.height;
    }
  ...
  private _sizeContainer(imageWidth: number, imageHeight: number): void {
    ...
    const newWidth = Number(imageWidth) + this._cssValue.containerRightPadding + this._cssValue.containerLeftPadding +
      this._cssValue.imageBorderWidthLeft + this._cssValue.imageBorderWidthRight;
    const newHeight = Number(imageHeight) + this._cssValue.containerTopPadding + this._cssValue.containerBottomPadding +
